#!/bin/bash
# File: run_experiments.sh (Query 3)
...
echo "=========================================="
echo "Query 3: Performance Experiments"
echo "=========================================="
...
for NUM_REDUCERS in 6 8 12 16; do
    ...
    # Clean output
    hdfs dfs -rm -r -f ${Q3_DIR}/exp_reducers_${NUM_REDUCERS}
    
    # Run job and time it
    START=$(date +%s)
    hadoop jar PopularLocationsByMonth.jar PopularLocationsByMonth \
        ${INPUT_CSV} \
        ${Q3_DIR}/exp_reducers_${NUM_REDUCERS} \
        ${NUM_REDUCERS} > /tmp/log_q3_reducers_${NUM_REDUCERS}.txt 2>&1
    EXIT_CODE=$?
    END=$(date +%s)
    DURATION=$((END - START))
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        ...
        echo "  ✗ Failed! Check /tmp/log_q3_reducers_${NUM_REDUCERS}.txt"
    fi
done
...
for SLOWSTART in 0.25 0.50 0.75 0.95; do
    ...
    # Clean output
    hdfs dfs -rm -r -f ${Q3_DIR}/exp_slowstart_${SLOWSTART}
    ...
    hadoop jar PopularLocationsByMonth.jar PopularLocationsByMonth \
        ${INPUT_CSV} \
        ${Q3_DIR}/exp_slowstart_${SLOWSTART} \
        12 > /tmp/log_q3_slowstart_${SLOWSTART}.txt 2>&1
    EXIT_CODE=$?
    END=$(date +%s)
    DURATION=$((END - START))
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        ...
        echo "  ✗ Failed! Check /tmp/log_q3_slowstart_${SLOWSTART}.txt"
    fi
done
...
echo "  - /Q3/reducer_experiment_results.csv"
echo "  - /Q3/slowstart_experiment_results.csv"
echo ""
